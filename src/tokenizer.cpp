...
std::vector<std::wstring> Tokenizer::tokenize(std::wstring str) {
    ...
    for (size_t i = 0, s = splits_by_unk.size(); i != s; ++i) {
        std::vector<std::wstring> tokenized = tokenize_without_unk(splits_by_unk[i]);
...
int main() {
    ...
    std::vector<std::wstring> tkn = tokenizer.tokenize(
        ...
        L"need to do well at school, get good marks in your tests and exams, and get more out of "
